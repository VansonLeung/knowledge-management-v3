"""Test webpage for MinerU document-to-markdown conversion.

Adds a simple HTML UI at /test for uploading PDFs/images and viewing results.
...
<head>
    ...
    <title>MinerU Service - Test</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        header { background: #8e44ad; color: white; padding: 1rem 1.5rem; flex-shrink: 0; }
        header h1 { font-size: 1.25rem; font-weight: 500; }
        header p { font-size: 0.875rem; opacity: 0.8; margin-top: 0.25rem; }
        header .badge { display: inline-block; background: rgba(255,255,255,0.2); padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.75rem; margin-left: 0.5rem; }
        ...
        .form-group input[type="file"]:hover { border-color: #8e44ad; background: #faf5fc; }
        .form-group select { width: 100%; padding: 0.5rem; border: 1px solid #ccc; border-radius: 6px; font-size: 0.875rem; }
        button { width: 100%; padding: 0.75rem; background: #8e44ad; color: white; border: none; border-radius: 6px; font-size: 1rem; cursor: pointer; font-weight: 500; }
        ...
        .result-card-header { padding: 0.75rem 1rem; background: #f3e5f5; border-bottom: 1px solid #ddd; font-weight: 500; display: flex; justify-content: space-between; align-items: center; }
        ...
        .tab:hover { color: #333; }
        .tab.active { color: #8e44ad; border-bottom-color: #8e44ad; }
        ...
        .note { font-size: 0.75rem; color: #888; margin-top: 0.5rem; padding: 0.5rem; background: #fff8e1; border-radius: 4px; }
    </style>
</head>
<body>
    <header>
        <h1>MinerU Service <span class="badge" id="backendBadge">Loading...</span></h1>
        <p>Upload PDFs or images to convert to Markdown with MLX acceleration</p>
    </header>
    <div class="container">
        <aside class="sidebar">
            <div class="sidebar-content">
                ...
                <form id="uploadForm">
                    <div class="form-group">
                        <label for="files">PDF / Image Files</label>
                        <input type="file" id="files" name="files" accept=".pdf,.png,.jpg,.jpeg,.tiff,.bmp" multiple>
                    </div>
                    <div class="form-group">
                        <label for="mode">Conversion Mode</label>
                        <select id="mode" name="mode">
                            <option value="convert">Convert to Markdown</option>
                            <option value="analyze">Full Analysis (with blocks)</option>
                        </select>
                    </div>
                    <button type="submit" id="submitBtn">Process Files</button>
                </form>
                <div class="note">⚡ First request downloads model weights (~10-20GB). Subsequent requests are fast.</div>
        ...
        <main class="results">
            ...
            <div class="results-content" id="resultsContent">
                <div class="empty-state">
                    ...
                    <p>Upload PDF or image files to see conversion results</p>
                </div>
            </div>
        </main>
    </div>
    <script>
        // Fetch health to show backend info
        fetch('/health').then(r => r.json()).then(data => {
            document.getElementById('backendBadge').textContent = `${data.backend} (concurrency: ${data.concurrency})`;
        }).catch(() => {
            document.getElementById('backendBadge').textContent = 'unknown';
        });
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const endpoint = mode === 'analyze' ? '/analyze' : '/convert/to-markdown';
        ...
        function renderResults() {
            ...
            resultsContent.innerHTML = results.map((r, idx) => {
                ...
                const pageCount = r.data.pages?.length || '?';
